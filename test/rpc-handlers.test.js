...
const mockRpc = {
  handlers: {},
  onWorkletStart: function (handler) {
    this.handlers.workletStart = handler
  },
  onGenerateEntropyAndEncrypt: function (handler) {
    this.handlers.generateEntropyAndEncrypt = handler
  },
  onGetMnemonicFromEntropy: function (handler) {
    this.handlers.getMnemonicFromEntropy = handler
  },
  onGetSeedAndEntropyFromMnemonic: function (handler) {
    this.handlers.getSeedAndEntropyFromMnemonic = handler
  },
  onInitializeWDK: function (handler) {
    this.handlers.initializeWDK = handler
  },
  onCallMethod: function (handler) {
    this.handlers.callMethod = handler
  },
  onDispose: function (handler) {
    this.handlers.dispose = handler
  },
}
...
describe('RPC Handlers', () => {
  ...
  describe('registerRpcHandlers', () => {
    test('should register all RPC handlers', () => {
      ...
      assert.ok(
        mockRpc.handlers.workletStart,
        'workletStart handler should be registered',
      )
      assert.ok(
        mockRpc.handlers.generateEntropyAndEncrypt,
        'generateEntropyAndEncrypt handler should be registered',
      )
      assert.ok(
        mockRpc.handlers.getMnemonicFromEntropy,
        'getMnemonicFromEntropy handler should be registered',
      )
      assert.ok(
        mockRpc.handlers.getSeedAndEntropyFromMnemonic,
        'getSeedAndEntropyFromMnemonic handler should be registered',
      )
      assert.ok(
        mockRpc.handlers.initializeWDK,
        'initializeWDK handler should be registered',
      )
      assert.ok(
        mockRpc.handlers.callMethod,
        'callMethod handler should be registered',
      )
      assert.ok(
        mockRpc.handlers.dispose,
        'dispose handler should be registered',
      )
    })
  })
  ...
  describe('generateEntropyAndEncrypt', () => {
    test('should generate entropy and encrypt for 12 words', async () => {
      registerRpcHandlers(mockRpc, context)
      const result = await mockRpc.handlers.generateEntropyAndEncrypt({
        wordCount: 12,
      })
      ...
      assert.ok(
        ...
        'encryptedSeedBuffer should be present',
      )
      assert.ok(
        result.encryptedEntropyBuffer,
        'encryptedEntropyBuffer should be present',
      )
    ...
    test('should reject invalid word count', async () => {
      ...
      await assert.rejects(
        async () =>
          await mockRpc.handlers.generateEntropyAndEncrypt({ wordCount: 15 }),
  ...
  describe('getMnemonicFromEntropy', () => {
    test('should decrypt entropy and return mnemonic', async () => {
      ...
      const generated = await mockRpc.handlers.generateEntropyAndEncrypt({
        wordCount: 12,
      })
    ...
    test('should reject invalid encrypted entropy', async () => {
      ...
      await assert.rejects(
        async () =>
          await mockRpc.handlers.getMnemonicFromEntropy({
        ...
        (error) => {
          ...
          return (
            errorStr.includes('encryptedEntropy') ||
  ...
  describe('getSeedAndEntropyFromMnemonic', () => {
    ...
    test('should reject invalid mnemonic', async () => {
      ...
      await assert.rejects(
        async () =>
          await mockRpc.handlers.getSeedAndEntropyFromMnemonic({
  ...
  describe('initializeWDK', () => {
    test('should initialize WDK with valid config', async () => {
      ...
      const mnemonic =
        'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about'
      const seedData = await mockRpc.handlers.getSeedAndEntropyFromMnemonic({
        mnemonic,
      })
  ...
  describe('callMethod', () => {
    ...
    test('should reject call when WDK not initialized', async () => {
      ...
      await assert.rejects(
        async () =>
          await mockRpc.handlers.callMethod({
            methodName: 'getAddress',
            network: 'ethereum',
            accountIndex: 0,
          }),
